...
const { EmbedBuilder, userMention, ActionRowBuilder, ButtonBuilder, ButtonStyle} = require("discord.js");
...
pay.callback(async ({inter}) => {
    ...
    if (users[1] < amount) {
        return await inter.followUp("You don't have enough money to pay that much! You currently have $" + users[1]);
    }
    else if (amount < 0) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`Are you sure you want to pay ${userMention(member.id)} $${amount}?`)
        .setColor('#F1C40F');
    ...
    collector.on("collect", async (i) => {
        if (i.customId === "confirm") {
            ...
            await i.update({
                embeds: [new EmbedBuilder().setTitle("Payment Cancelled").setDescription(`You cancelled the payment to ${userMention(member.id)}`).setColor('#F1C40F')],
                components: [],
            });
        }
    });
    collector.on("end", async (collected, reason) => {
        if (reason === "time") {
            await msg.edit({
                embeds: [new EmbedBuilder().setTitle("Payment Cancelled").setDescription(`You took too long to confirm the payment to ${userMention(member.id)}`).setColor('#F1C40F')],
                components: [],
            });
        }
    }
