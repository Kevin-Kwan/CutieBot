...
const balance = new SlashCommand()
  ...
  .addUserOption((option) =>
    option.setName('member').setDescription('target @member').setRequired(false)
  )
...
balance.callback(async ({ inter }) => {
  ...
  const net_amt = wallet_amt + bank_amt
...
withdraw.callback(async ({ inter }) => {
  ...
  if (amount > bank_amt) {
    return await inter.followUp(
      `${userMention(
        user.id
      )} You don't have enough money! You have $ ${bank_amt} in your bank!`
    )
  }
  if (amount < 0) {
    return await inter.followUp(
...
deposit.callback(async ({ inter }) => {
  ...
  if (amount > wallet_amt) {
    return await inter.followUp(
      `${userMention(
        user.id
      )} You don't have enough money! You have $ ${wallet_amt} in your wallet!`
    )
  }
  if (amount < 0) {
    return await inter.followUp(
...
send.callback(async ({ inter }) => {
  ...
  if (member.bot) return await inter.followUp("Bots don't have account")
  ...
  await inter.followUp(`You sent ${amount} to ${userMention(member.id)}`)
})
...
leaderboard.callback(async ({ inter }) => {
  ...
  for (const member of users) {
    ...
    const memberDetails = await inter.client.users.cache.find(
      (u) => u.id == member[0]
    )
...
add_money.callback(async ({ inter }) => {
  ...
  await update_bank(member, +amount)
  await inter.followUp(`Added $ ${amount} to ${userMention(member.id)}`)
})
...
remove_money.callback(async ({ inter }) => {
  ...
  await update_bank(member, -amount)
  await inter.followUp(`Removed $ ${amount} from ${userMention(member.id)}`)
})
