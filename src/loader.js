...
readdirSync('./src/slash-commands/').forEach((dirs) => {
  const commands = readdirSync(`./src/slash-commands/${dirs}`).filter((files) =>
    files.endsWith('.js')
  )
  ...
  for (const file of commands) {
    ...
    if (command.name && command.description) {
      ...
      console.log(
        `-> [Loaded Old Slash Command] ${command.data.name.toLowerCase()}`
      )
      delete require.cache[
        require.resolve(`../src/slash-commands/${dirs}/${file}`)
      ]
...
client.on('ready', async (client) => {
  ...
  guildIds.forEach((guild) => {
    ...
    if (
      ...
      client.guilds.cache.get(guild).commands.cache.size > 0
    ) {
