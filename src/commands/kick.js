module.exports = {
    ...
    if (message.member.permissionsIn(message.channel).has("KICK_MEMBERS")) {
        ...
        user.kick({reason}).then((user) => {
            if (!reason) {
            reason = "No reason provided."
            }

                let formattedReason = `\`\`${reason}\`\``;
                message.channel.send(""+user.displayName + " has been successfully kicked for: " + formattedReason);
        }).catch((error) => {
            console.log(error);
            message.channel.send("You do not have permissions to kick "+ args[0]+".");
        });
    } else
        {
